...
namespace capaVista
{
    public partial class frmCliente : Form
    {
        ...
        public frmCliente()
        {
            ...
            this.ttMensaje.SetToolTip(this.txtNum_Documento, "Ingrese ingrese el numero de documento del Cliente");
        ...
        {
            // TODO: This line of code loads data into the 'dbventasDataSet.spmostrar_cliente' table. You can move, or remove it, as needed.
            this.spmostrar_clienteTableAdapter.Fill(this.dbventasDataSet.spmostrar_cliente);
            this.Top = 0;
            this.Left = 0;
            this.Mostrar();
            this.Habilitar(false);
            this.Botones();
            this.txtIdcliente.Hide();
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dataListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            if (respuesta.Equals("Se eliminó correctamente"))
                            {
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            {
                string respuesta = "";
                if (this.txtNombre.Text == string.Empty || this.txtNum_Documento.Text == string.Empty || this.txtDireccion.Text == string.Empty)
                {
                ...
                else
                {
                    if (this.IsNuevo)
                    {
                        respuesta = NCliente.Insertar(txtNombre.Text.Trim().ToUpper(),
                            txtApellidos.Text, cbSexo.Text, dtpFecha_Nacimiento.Value, cbTipo_Documento.Text, txtNum_Documento.Text,
                            txtDireccion.Text, txtTelefono.Text, txtEmail.Text);

                    }
                    else
                    {
                        respuesta = NCliente.Editar(Convert.ToInt32(this.txtIdcliente.Text), txtNombre.Text.Trim().ToUpper(),
                            txtApellidos.Text, cbSexo.Text, dtpFecha_Nacimiento.Value, cbTipo_Documento.Text, txtNum_Documento.Text,
                            txtDireccion.Text, txtTelefono.Text, txtEmail.Text);
                    }

                    if (respuesta.Equals("Se editó correctamente")||respuesta.Equals("Se ingreso correctamente"))
                    {
                        if (this.IsNuevo)
                        {
                            this.MensajeOk("Se ingresó de forma correcta el registro");
                        }
                        else
                        {
